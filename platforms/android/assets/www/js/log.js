...
var Loger= {
    ...
    write:function (data,fileName){
        Loger.datas=data;
        Loger.fileName=fileName;
        window.resolveLocalFileSystemURL(cordova.file.externalDataDirectory,Loger.onDirectorySuccess, Loger.onFileSystemFail);
},
...
//获取mobovip目录下面的stores.txt文件，如果不存在则创建此文件
    ...
    newFile.getFile(Loger.fileName, {
        create : true,
        exclusive : false
    }, Loger.onFileSuccess, Loger.onFileSystemFail);
},
/**
...
onFileSuccess:function (fileEntry) {
    fileEntry.createWriter(Loger.onFileWriterSuccess,Loger.onFileSystemFail);
},
...
//  log("fileName="+writer.fileName+";fileLength="+writer.length+";position="+writer.position);
    ...
    writer.write("/n"+Loger.datas);//向文件中写入数据
